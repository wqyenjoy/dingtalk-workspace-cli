...
- 用户明确指定回复、本地文件或禁止创建/发送时优先遵守，该约束覆盖下方 recipe 的默认交付步骤。
...
| generate-weekly-report | 1. 按[「多源并行采集」](./report-conventions.md#多源并行采集公共模式)执行，优先用户给定时间范围，未给才用本周<br>2. 按真实来源及时间依据归纳，列出缺失或不完整来源<br>3. 按用户要求交付：仅回复则直接输出，不创建或发送；要求本地文档则保存并验证文件；要求在线文档则切 dingtalk-doc 创建及验证 |

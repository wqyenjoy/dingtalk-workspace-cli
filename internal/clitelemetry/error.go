...
func SanitizeErrorText(message string) string {
	...
	return TruncateText(message, MaxErrorRunes)
}
...
func TruncateText(message string, maxRunes int) string {
	if maxRunes <= 0 {
		return ""
	}
	runes := []rune(message)

...
// SnapshotProjectors permits focused invariant tests to substitute one pure
// projection without changing the registry model.
...
func ToolProvenanceValue(tool ToolSpec, field string) (any, bool) {
	return tool.provenanceValue(field)
}
...
func ParameterProvenanceValue(parameter ParameterSpec, field string) (any, bool) {
	return parameter.provenanceValue(field)
}
...
func ProductProvenanceValue(product ProductSpec, field string) (any, bool) {
	return product.provenanceValue(field)
}
...
func (r SchemaRegistry) ToSnapshotPayload() (SchemaSnapshotPayload, error) {
	return ToSnapshotPayloadWithProjectors(r, SnapshotProjectors{})
}

// ToSnapshotPayloadWithProjectors derives a snapshot with optional pure
// projector substitutions. Nil projectors use the standard wire projection.
func ToSnapshotPayloadWithProjectors(r SchemaRegistry, projectors SnapshotProjectors) (SchemaSnapshotPayload, error) {
	...
	for _, product := range r.Products {
		...
		for _, tool := range product.Tools {
			renderSummary := projectors.ToolSummary
			if renderSummary == nil {
				renderSummary = ToolSpec.ToSummaryPayload
			}
			summary, summaryErr := renderSummary(tool)
			if summaryErr != nil {
				return SchemaSnapshotPayload{}, summaryErr
			}
			renderTool := projectors.ToolPayload
			if renderTool == nil {
				renderTool = ToolSpec.ToPayload
			}
			full, fullErr := renderTool(tool)
			if fullErr != nil {
		...
		renderProduct := projectors.ProductSummary
		if renderProduct == nil {
			renderProduct = ProductSpec.ToSummaryPayload
		}
		productPayload, productErr := renderProduct(product)
		if productErr != nil {
...
func (t ToolSpec) normalized() ToolSpec {
	...
	out.Constraints = contract.NormalizeRuntimeSchemaConstraints(t.Constraints)
	if t.DryRun != nil {
		dryRun := *t.DryRun
...
func NormalizeToolSpec(tool ToolSpec) ToolSpec { return tool.normalized() }
...
func cloneFieldProvenance(source map[string]contract.FieldProvenance) map[string]contract.FieldProvenance {
	return CloneFieldProvenance(source)
}
...
func (t ToolSpec) ToPayload() (map[string]any, error) {
	...
	if !contract.RuntimeSchemaConstraintsEmpty(t.Constraints) {
...
func DefaultString(value, fallback string) string { return defaultString(value, fallback) }
...
func StableUniqueStrings(values []string) []string { return stableUniqueStrings(values) }

// SortedUniqueStrings exposes the model's sorted normalization.
func SortedUniqueStrings(values []string) []string { return sortedUniqueStrings(values) }

// CloneOptionalStrings defensively copies a list while preserving nil versus empty.
func CloneOptionalStrings(values []string) []string { return cloneOptionalStrings(values) }

// RawJSONValue decodes one validated polymorphic JSON value.
func RawJSONValue(raw json.RawMessage) (any, error) { return rawJSONValue(raw) }

// TypedJSONValue projects a typed value into its plain JSON representation.
func TypedJSONValue(value any) (any, error) { return typedJSONValue(value) }

// ValidateFinalFieldProvenance verifies that provenance selects the delivered value.
func ValidateFinalFieldProvenance(owner, field string, provenance contract.FieldProvenance, finalValue any) error {
	return validateFinalFieldProvenance(owner, field, provenance, finalValue)
}

// EqualJSONValues compares JSON values using the model's number-preserving semantics.
func EqualJSONValues(left, right []byte) bool { return equalJSONValues(left, right) }
